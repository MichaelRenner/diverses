...
XML_FILE = "fritzbox5_phonebook.xml"
PHONEBOOK_NAME = "Hamnet ohne Prefix"
PREFIX = "*124#"  # Beispielpräfix für Deutschland
    ...
    for entry in entries:
        ...
        if "cn" in attr and "telephoneNumber" in attr:
            ...
            for number in attr["telephoneNumber"]:
                # Bereinigung der Nummer und Präfix hinzufügen
                clean_number = number.decode("utf-8") if isinstance(number, bytes) else str(number)
                number_element = etree.SubElement(telephony, "number")
                number_element.text = f"{PREFIX}{clean_number}"  # Präfix und bereinigte Nummer
